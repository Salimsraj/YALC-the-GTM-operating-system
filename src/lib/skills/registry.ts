...
export async function registerBuiltinSkills(registry: SkillRegistry): Promise<void> {
  ...
  const { replyToCommentsSkill } = await import('./builtin/reply-to-comments')
  const { findPeopleSkill } = await import('./builtin/find-people')
  const { findLinkedinSkill } = await import('./builtin/find-linkedin')
  const { trackCampaignSkill } = await import('./builtin/track-campaign')
  const { researchSkill } = await import('./builtin/research')
  const { lostDealRevivalSkill } = await import('./builtin/lost-deal-revival')
  const { crustdataApiSkill } = await import('./builtin/crustdata-api')
  const { fullenrichApiSkill } = await import('./builtin/fullenrich-api')
  const { firecrawlApiSkill } = await import('./builtin/firecrawl-api')
  const { instantlyApiSkill } = await import('./builtin/instantly-api')
  ...
  registry.register(replyToCommentsSkill)
  registry.register(findPeopleSkill)
  registry.register(findLinkedinSkill)
  registry.register(trackCampaignSkill)
  registry.register(researchSkill)
  registry.register(lostDealRevivalSkill)
  registry.register(crustdataApiSkill)
  registry.register(fullenrichApiSkill)
  registry.register(firecrawlApiSkill)
  registry.register(instantlyApiSkill)
